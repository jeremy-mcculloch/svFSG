...
//     int sn = curr_vessel->sn;
//     double a_store = curr_vessel->a[sn];
...
double iv_obj_f(double a_mid_guess_inner, void *input_vessel) {
    ...
    double mu = 0.0;
    double pa_calc = 0.0;
    double fz_total = 0.0; bool equil_check = !(sn > 0 || curr_vessel->num_exp_flag == 1);
    ...
    for (int layer = 0; layer < n_layers; layer ++) {
        ...
        if (layer == 0) {
            lambda_t = a_mid_guess_inner / curr_vessel->layers[layer].a_mid[0];
